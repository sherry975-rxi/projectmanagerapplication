...
export function handleProjectHeaders(list) {
    return (
        list.map((project) => [project.projectId, project.ProjectManager, project.description, project.status, formatDate(project.startDate), '']
        )
    )
}
...
export function handleUserHeaders(list) {
    return (
        list.map((user) => [user.name, user.email, user.userProfile, user.function, user.systemUserStateActive, ''])
    )
}
...
function formatDate(date) {
    ...
    let formatedDate = new Date(mydate.getTime() - mydate.getTimezoneOffset() * -60000)
